{
  ...
  dashboards = {
    ...
    "matrix-remote" = dashboard "matrix-remote" "Matrix Remote Server" ["matrix" "remote" "host"] [
      ...
      (timeseriesPanel 11 "Key service state" 0 20 24 8 [(prometheusTarget "A" ''node_systemd_unit_state{instance="${matrixHost}", state="active", name=~"authentik.service|authentik-worker.service|bluesky-pds.service|cadvisor.service|docker.service|forgejo-runner-codeberg.service|matrix-synapse.service|nginx.service|postgresql.service|prometheus-nginx-exporter.service|prometheus-postgres-exporter.service|prometheus-redis-exporter.service|redis-authentik.service|shop-empty-track.service|sshd.service|tailscaled.service|atm10-7-2.service"}'' "{{name}}")] "none")

---
name: recall
description: "Use only when the user asks to recall recent work, catch them up, summarize where they left off, or reconstruct context before resuming a task."
---
...
Keep it tight and on-topic. Read only what the in-scope threads need, then stop. Fan out heavy reading only when the corpus is large enough to benefit. The main thread keeps only the findings and the final brief.
...
Use Codex thread inspection tools when available. Otherwise scan `~/.codex/sessions/` and `~/.codex/archived_sessions/` for the active project path.
...
3. Fan out across your chat history only when the corpus is large enough to benefit. Use Codex collaboration tools with bounded slices and the parent model. Tell every subagent to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.

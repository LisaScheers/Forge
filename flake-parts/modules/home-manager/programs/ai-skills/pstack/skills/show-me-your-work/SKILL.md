...
At the end of the run, before handing back, check the log told the truth. Use Codex thread inspection tools when available. Otherwise read only this workspace's matching transcript under `~/.codex/sessions/` or `~/.codex/archived_sessions/`; do not scan unrelated projects. Walk the log against what actually happened:
...
For high-stakes or unattended work, spawn an independent Codex subagent when the user requested parallel review or the task genuinely benefits from it. Inherit the parent model unless the user or repository configuration requests another available model. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. This is not a redo of the work, but a scan for what is suboptimal or risky.
...
When an independent review ran, end the reply with an "Attention" section listing each flag and the specific row or moment it concerns. "No flags" is valid. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.

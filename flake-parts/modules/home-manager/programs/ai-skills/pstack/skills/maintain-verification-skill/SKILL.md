...
description: "Use only when the user invokes $maintain-verification-skill or explicitly asks to audit or update a project's verification skill and feature map."
---
...
- **changed** — proven doc, harness, or map corrections are ready for review.
...
6. **Ship or stop.** For changed: leave the proven corrections as a reviewable diff and re-read every changed file first. Open one PR with `$file-pr` only when the user asks. For clean or blocked: report the outcome and the coverage honestly.

...
3. Migrate one component at a time. When several components are truly independent and delegation is justified, give each owner exclusive files. Migrate shared primitives first.
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. schedules per component until the diff is zero.
5. Run **Opening a PR** only when the user asked for a pull request.

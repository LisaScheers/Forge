...
3. Plan the fix from the trace. Use `architect` only for a hard-to-reverse boundary or ownership change. Delegate an independent bounded implementation only when coordination is justified. Inherit the parent model unless repository configuration supplies a supported model and reasoning effort separately. Review delegated diffs and capture a post-fix trace.
   Apply the **sequence-verifiable-units** principle skill, verifying each attempt before trying the next.
4. Parse and compare the artifacts (JSON to sqlite, diff). "Inconclusive" or wrong-surface is not a pass; flag it.
5. If a PR was requested, cite the measurement in its description and run **Opening a PR**.

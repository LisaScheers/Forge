...
Use `update_plan` with one entry per phase before starting. Autonomous mode without checkpoints needs the plan to show phase position and keep phases from silently disappearing.
...
Use the parent model unless a repository configuration names models accepted by `collaboration.spawn_agent`.

...
**Start every multi-step task with `update_plan`, whose first item is to read the Principles section below in full.** The principles ground every trigger here. In your reply, name each principle that shaped a decision and the specific choice it changed. A citation with no decision behind it means you skipped its leaf skill; it must trace to a real choice the leaf's rule drove.
...
- Before a requested commit → the **unslop** skill.
- Before review → the **no-comments** skill (`$no-comments`).
- Shipping UI / IDE / CLI → the matching available control skill or tool. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
- Any PR-status request → the **babysit-pr** skill. Never trigger it merely because a PR was opened.
- Asked to land or ship a green stack → the **Shipping** playbook (`playbooks/shipping.md`). Green is not safe. Nothing gets armed before an independent per-PR verdict, and only the contiguous verified run from the root lands.
- Bugbot or the agentic security review commented → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `references/bugbot-triage.md`.
- Broken skill mid-task → report it and use a safe fallback. Fix it only when the user asked to change skills.
- Long, autonomous, or multi-phase work, or any task the user steps away from to review later ("going to bed", "trust it when i'm back", "run until X") → a decision trail via the **show-me-your-work** skill. Commit it only when the user asked for a commit; keep it local otherwise.
...
**Just do in-scope work.** Use read-only tools freely. Reversible changes proceed when they are part of the user's requested task.

**Always pause** for actions outside the requested scope and external writes such as messages, ticket changes, commits, pushes, PR changes, merges, deploys, and data deletion.
...
Use Codex collaboration tools for bounded work that benefits from delegation. Inherit the parent model unless the user or repository configuration explicitly requests another available model. Do not delegate ordinary single-pass tasks.
...
Every playbook ends with a reply written this way. Include a PR link only when the requested workflow opened one.
...
Treat the copied playbooks as workflow guidance, not literal Cursor tool syntax. Translate `todolist` to `update_plan`, `Task` or cloud-agent calls to Codex collaboration tools, `AskQuestion` to asking the user, `create-skill` to `$skill-creator`, `deslop` to `$unslop`, and `/loop` to the available monitoring mechanism. The bundled Bun scripts are upstream helpers, not required Codex dependencies; prefer Codex plans, collaboration, and `$babysit-pr`. No playbook expands the user's authorization.

Your first plan actions are the matched playbook's steps, before any task-specific items and before you reason about the task. The failure mode is reading a playbook then writing a bespoke plan that drops its named steps (`architect`, the throughput checkpoint). A step you choose not to do stays in the list with a one-line `skip: <reason>`; skipping silently is not allowed. Match the task to a playbook below and open its file.
...
- **Opening a PR.** Invoke only when the user asks to open a PR, then use `$file-pr`. `playbooks/opening-a-pr.md`.

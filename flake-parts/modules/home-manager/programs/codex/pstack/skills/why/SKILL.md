...
Before spawning investigators, inspect the available Codex tools and connectors. When the tool catalog is exposed through `ALL_TOOLS`, use it. Do not assume a category is available and do not scan product-specific configuration directories.
...
- Spawn with `collaboration.spawn_agent` and `fork_turns: "none"`.
- `model`: your configured why-investigators model (upstream default `gpt-5.6-terra-xhigh`) when Codex accepts it; otherwise omit the model override to inherit the parent model.
- Tell the investigator to make no edits or external writes. Codex collaboration has no `readonly` field; this is an instruction, not a sandbox.
...
Spawn one synthesizer subagent with `collaboration.spawn_agent` and `fork_turns: "none"`:

- `model`: your configured why-synthesizer model (upstream default `claude-fable-5-thinking-max`) when Codex accepts it; otherwise omit the model override to inherit the parent model.
- Tell the synthesizer to make no edits or external writes. It may use available read-only tools to spot-check citations.

...
Spawn the explorers with `collaboration.spawn_agent`, using `fork_turns: "none"` and the parent model. Tell each explorer to make no edits.
...
Explore and explain directly in the current task. Read `references/explainer-prompt.md` for the communication style and output format. Use the same structure, just without explorer findings as input.

Proceed to Step 4.

### Step 3. Synthesize (complex questions only)

Synthesize the explorers' findings in the current task. Read `references/explainer-prompt.md` for the output format. Reconcile overlaps and contradictions, then weave the slices into one explanation.
...
After the explanation is complete, spawn the requested architectural critics with `collaboration.spawn_agent`, using `fork_turns: "none"` and the parent model.

Tell each critic to make no edits.
